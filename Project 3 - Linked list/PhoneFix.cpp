...
bool PhoneBook::Insert(const string& last, const string& first, const string& ph)
{
	...
	{ // the last name given comes before the last name alphabetically, so make the person the head of the list make one if statement: (last < least || last = last && first < first)
	...
	else if (last == head->lastname && first < head->firstname) // this has a problem
	{
		PhoneBookItem* p = new PhoneBookItem(last, first, ph);
		p->next = head;
		head = p;
	...
	/*
	if (last == head->lastname && first < head->firstname)
	{
		PhoneBookItem* p = new PhoneBookItem(last, first, ph);
		p->next = head;
		head = p;
...
{
	bool answer;
	PhoneBookItem* location = head; // q
	PhoneBookItem* tempLocation; // p
	...
	else 
	{
		...
		while (head != NULL && last > head->lastname)
		{
			tempLocation = head;
			head = head->next;
		}
		// now if location points to the right place, delete it
		if (head != NULL && last == head->lastname && first == head->firstname)
		{ // delete the node pointed to by location from the list
			tempLocation = head;
			head = head->next; // deletes the node
			delete tempLocation;
			num--;
			answer = true;
		}
